"""Medication: a consumable inventory item used for animal treatment.
...
class Medication:
    """Medication - one type of medication stocked in an Inventory.
        ...
        - _id (int | None): primary key once persisted, None for a
          Medication that has not been saved yet.
        - _name (str): e.g. "Antibiotic A".
        - _quantity (float): current stock, in whatever unit the
          medication is dosed in (doses, ml, ...).
    ...
    def decrease_quantity(self, amount: float) -> bool:
        """Consume stock, e.g. when Veterinarian.treat_animal() uses this.
